...
void target_console_init(void){
    /* Enable UART clocks */
    rcc_periph_clock_enable(CONSOLE_TX_USART_CLOCK);
    rcc_periph_clock_enable(CONSOLE_RX_USART_CLOCK);

    /* Setup GPIO pins */
    gpio_set_mode(CONSOLE_TX_USART_GPIO_PORT, GPIO_MODE_OUTPUT_50_MHZ,
                  GPIO_CNF_OUTPUT_ALTFN_PUSHPULL, CONSOLE_USART_GPIO_TX);
    gpio_set_mode(CONSOLE_RX_USART_GPIO_PORT, GPIO_MODE_INPUT,
